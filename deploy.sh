...
current_pod=$(kubectl get pods --no-headers | awk '/waltid-digital-credentials/ {print $1; exit 0}')
...
new_pod=""
for i in {1..60}; do
  new_pod=$(kubectl get pods --no-headers | awk '/waltid-digital-credentials/ {print $1; exit 0}')
  if [ -n "${new_pod}" ]; then
    # Ensure it's ready before proceeding.
    if kubectl get pod "${new_pod}" 2>/dev/null | awk 'NR==2 {print $2}' | grep -q '^1/1$'; then
      status=$(kubectl get pod "${new_pod}" -o jsonpath='{.status.phase}')
