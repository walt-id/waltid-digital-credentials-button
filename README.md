...
- `@waltid/dc-backend-demo` — Lightweight dev-server middleware that creates verifier sessions from `apps/config/*-conf.json` and exposes `/api/dc/request/:id` and `/api/dc/response`.
...
## Endpoints & configs
- Request endpoint: `/api/dc/request/:requestId` (also accepts `?request-id=`). Uses `apps/config/<id>-conf.json` to create a verifier session and return the DC API payload.
